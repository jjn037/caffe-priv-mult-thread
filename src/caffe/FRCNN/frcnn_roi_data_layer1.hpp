...
//  std::string default_config_file = this->layer_param_.windows_data_param().config();
//  FrcnnParam::load_param(default_config_file);
//  FrcnnParam::print_param();
  cache_images_ = this->layer_param_.windows_data_param().cache_images();

  LOG(INFO) << "FrcnnRoiDataLayer :" ;
  LOG(INFO) << "  source file :"
            << this->layer_param_.windows_data_param().source() ;
  LOG(INFO) << "  cache_images: "
            << ( cache_images_ ? "true" : "false" ) ;
  LOG(INFO) << "  root_folder: "
            << this->layer_param_.windows_data_param().root_folder() ;
  ...
  std::ifstream infile(this->layer_param_.windows_data_param().source().c_str());
