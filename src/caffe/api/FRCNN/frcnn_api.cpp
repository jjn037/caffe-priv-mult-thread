...
void Detector::Set_Model(int gpu_id) {
//FrcnnParam::load_param("/home/priv-lab1/workspace/faster_exp/team2/faster-2fc/faster.json");
caffe::Caffe::SetDevice(gpu_id);
caffe::Caffe::set_mode(caffe::Caffe::GPU);
std::string proto_file             = FrcnnParam::det_proto;
std::string model_file             = FrcnnParam::det_weight;
  ...
  std_[0] = FrcnnParam::pixel_stds[0];
  std_[1] = FrcnnParam::pixel_stds[1];
  std_[2] = FrcnnParam::pixel_stds[2];
...
void Detector::predict(const cv::Mat &img_in, std::vector<caffe::Frcnn::BBox<float> > &results, boost::shared_ptr<Blob<float> > &img_crop) {
  ...
  if (FrcnnParam::iter_test == -1) {
    predict_original(img_in, results, img_crop);
  } else {
    predict_iterative(img_in, results);
  }
}

void Detector::predict_original(const cv::Mat &img_in, std::vector<caffe::Frcnn::BBox<float> > &results, boost::shared_ptr<Blob<float> > &img_crop) {
  ...
  img_in.convertTo(img_original, CV_32FC3);
  for (int r = 0; r < img.rows; r++) {
    for (int c = 0; c < img.cols; c++) {
      ...
      reinterpret_cast<float *>(img.data)[offset + 0] /= this->std_[0]; // B
      reinterpret_cast<float *>(img.data)[offset + 1] /= this->std_[1]; // G
      reinterpret_cast<float *>(img.data)[offset + 2] /= this->std_[2]; // R
    }
  }
  ...
  for (int r = 0; r < img.rows; r++) {
    for (int c = 0; c < img.cols; c++) {
      int offset = (r * img.cols + c) * 3;
      reinterpret_cast<float *>(img_original.data)[offset + 0] -= this->reid_mean_[0]; // B
      reinterpret_cast<float *>(img_original.data)[offset + 1] -= this->reid_mean_[1]; // G
      reinterpret_cast<float *>(img_original.data)[offset + 2] -= this->reid_mean_[2]; // R

      reinterpret_cast<float *>(img_original.data)[offset + 0] /= this->reid_std_[0]; // B
      reinterpret_cast<float *>(img_original.data)[offset + 1] /= this->reid_std_[1]; // G
      reinterpret_cast<float *>(img_original.data)[offset + 2] /= this->reid_std_[2]; // R
    }
  }
  ...
  this->preprocess(img_original, 1);
  this->preprocess(im_info, 2);

  vector<std::string> blob_names(4);
  blob_names[0] = "rois";
  blob_names[1] = "cls_prob";
  blob_names[2] = "bbox_pred";
//  blob_names[3] = "img_crop";
...
//  boost::shared_ptr<Blob<float> > _img_crop(output[3]);

//  const int count = _img_crop->count();
//  img_crop=_img_crop;

//  std::cout<<_img_crop->num()<<" "<<_img_crop->channels()<<" "<<_img_crop->height()<<" "<<_img_crop->width()<<std::endl;
